  ...
  )
{
  ...
  UINTN InfSize;
  UINT32 Mode = 0, DesiredHorizontalResolution = 2000, 
          DesiredVerticalResolution = 1000, MaxMode = GraphicsOutput->Mode->MaxMode;
  while (Mode < MaxMode) {
    ...
    if (inf->HorizontalResolution > DesiredHorizontalResolution ||
            inf->VerticalResolution > DesiredVerticalResolution) {
      if (Mode > 0) {
        --Mode;
      }
      break;         
  ...
  DEBUG ((DEBUG_INFO, "JOS: Graphics Mode set %d\n", Mode));
